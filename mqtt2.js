import { 
  ...
  let msg = null
  let prefix = ''
  const mqttConnectionTestsErrorMsg = {statusCode: 400, message: 'Shelly device MQTT not connected - Check configuration'}
  const clientOnErrorMsg = {statusCode:400, message:"Can't connect to MQTT Server"}
  ...
  const mqttConnectionTests = async (devIP) => {
    ...
    if (testMqtt > 0){
      console.error("Shelly device MQTT not connected - Check configuration - Error:", testMqtt)
      return 2
    } else {
  ...
  const mqttConnection = async (SIP,BIP) => {
    ...
    if(!connected){
        ...
        if(!reconnected.restart_required)
            return 1
        else
            return 2
    }
  ...
  const getTopicMsg = async (topic, collection) => {
    const client = mqtt.connect(`mqtt://${BROKER_IP}`)
      ...
      client.on('message', async (topic, data) => {
        ...
        if(collection === 0){
          try{
            docObj = {device:prefix, timestamp: Date.now(), 
              online:obj1
            }
          } catch(e) {          
            console.log("ERROR",e)
        ...
        if(collection === 1){
          ...
          // console.log(obj2)
          try{
            docObj = {device:obj1.src, timestamp:obj1.params.ts, 
        ...
        if(collection === 2){
          ...
          try {
            docObj = {
              device: obj1.src, timestamp: Date.now(), 
  ...
  const mqttMsgToMongo = async (inputJson,collection) => {
    ...
    try{
      return new Promise(async(resolve,reject) => {
        ...
        if(insertedId){
            resolve({statusCode: 200, message: "Document created", insertedId: insertedId})
        } else {
            reject({statusCode: 201, message: "Document not created in the Database"})
        }
      })
    } catch (e) {
  ...
  export async function getDeviceData(data) {
    ...
    let results = ''
        let funRes = await mqttMsgToMongo(data);
        console.log(funRes)
    try {
